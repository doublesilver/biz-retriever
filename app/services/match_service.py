...
class HardMatchEngine:
    ...
    def _check_region(
        ...
        if profile.region_code == bid_region:
            ...
            return (
                False,
                f"지역 불일치: 입찰({self.REGION_CODES.get(bid_region, bid_region)}) vs 사용자({self.REGION_CODES.get(profile.region_code, profile.region_code)})",
            )
    ...
    def _get_max_performance(self, profile: UserProfile) -> float:
        ...
        amounts = [
            perf.contract_amount
            for perf in profile.performances
            if perf.contract_amount
        ]

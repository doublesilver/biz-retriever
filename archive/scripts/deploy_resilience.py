...
def main():
    print("🚀 Deploying Service Resilience Update...")
    
    # Update services
    deploy_file('app/services/crawler_service.py', f'{REMOTE_PROJECT_DIR}/app/services/crawler_service.py')
    deploy_file('app/services/rag_service.py', f'{REMOTE_PROJECT_DIR}/app/services/rag_service.py')
    deploy_file('requirements.txt', f'{REMOTE_PROJECT_DIR}/requirements.txt')

    # Restart API and Worker to apply changes
    print("\n🔄 Restarting API and Worker...")
    run_ssh(f"cd {REMOTE_PROJECT_DIR} && docker compose restart api celery-worker")
        
    print("\n✅ Resilience Update Deployed!")
